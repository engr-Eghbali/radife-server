...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
func schedule_ctrl(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		customer := r.Form["customer"][0]
		//key :=r.Form["key"][0]
		timeOut := r.Form["timeOut"][0]
		dateOut := r.Form["dateOut"][0]
		name := r.Form["name"][0]

		flg := req.Schedule(customer, timeOut, dateOut, name)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
func showSchedule_ctrl(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		var resp string
		//key :=r.Form["key"][0]
		results, flg := req.ShowSchedule(customer)

		if flg {

			for i, item := range results {

				resp = resp + " <div class=\"hisrow\" id=\"hisrowgood" + item.ID.Hex() + "\"><p1>" + strconv.Itoa(i) + "</p1><p2>" + item.DateOut + "<br>" + item.TimeOut + "</p2><p3>" + item.Total + "تومان</p3><p4 onclick=\"cancelSchedule('" + item.ID.Hex() + "')\"><i class=\"fas fa-trash-alt\" style=\"color:black;\"> </i></p4></div>"
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
func cancelSchedule_ctrl(w http.ResponseWriter, r *http.Request) {
...
func main() {
	...
	schedule_rout := secure.New(secure.Options{
		AllowedHosts:            []string{"ssl.example.com"},                     // AllowedHosts is a list of fully qualified domain names that are allowed. Default is empty list, which allows any and all host names.
		HostsProxyHeaders:       []string{"X-Forwarded-Hosts"},                   // HostsProxyHeaders is a set of header keys that may hold a proxied hostname value for the request.
		SSLRedirect:             true,                                            // If SSLRedirect is set to true, then only allow HTTPS requests. Default is false.
		SSLTemporaryRedirect:    false,                                           // If SSLTemporaryRedirect is true, the a 302 will be used while redirecting. Default is false (301).
	...
	showSchedule_rout := secure.New(secure.Options{
		AllowedHosts:            []string{"ssl.example.com"},                     // AllowedHosts is a list of fully qualified domain names that are allowed. Default is empty list, which allows any and all host names.
		HostsProxyHeaders:       []string{"X-Forwarded-Hosts"},                   // HostsProxyHeaders is a set of header keys that may hold a proxied hostname value for the request.
		SSLRedirect:             true,                                            // If SSLRedirect is set to true, then only allow HTTPS requests. Default is false.
		SSLTemporaryRedirect:    false,                                           // If SSLTemporaryRedirect is true, the a 302 will be used while redirecting. Default is false (301).
	...
	cancelSchedule_rout := secure.New(secure.Options{
		AllowedHosts:            []string{"ssl.example.com"},                     // AllowedHosts is a list of fully qualified domain names that are allowed. Default is empty list, which allows any and all host names.
		HostsProxyHeaders:       []string{"X-Forwarded-Hosts"},                   // HostsProxyHeaders is a set of header keys that may hold a proxied hostname value for the request.
		SSLRedirect:             true,                                            // If SSLRedirect is set to true, then only allow HTTPS requests. Default is false.
		SSLTemporaryRedirect:    false,                                           // If SSLTemporaryRedirect is true, the a 302 will be used while redirecting. Default is false (301).
	...
	app16 := schedule_rout.Handler(http.HandlerFunc(schedule_ctrl))
	app17 := showSchedule_rout.Handler(http.HandlerFunc(showSchedule_ctrl))
	app18 := cancelSchedule_rout.Handler(http.HandlerFunc(cancelSchedule_ctrl))
	...
	http.Handle("/schedule", app16)
	http.Handle("/showSchedule", app17)
	http.Handle("/cancelSchedule", app18)
